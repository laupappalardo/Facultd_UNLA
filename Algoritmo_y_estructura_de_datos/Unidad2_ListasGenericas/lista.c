...
/*void ordenarLista(ListaPtr lista){
    ...
    for(int i=0; i<tam; i++){
        for (int j=0; j<tam-1; j++){
            ...
            if(getDato(actual) > getDato(getNodoSiguiente(actual))){
                aux=getDato(actual);
...
ListaPtr duplicarLista(ListaPtr lista){
    ...
    insertarPrimero(listaCopia, getDato(actual));
    for (int i=1; i<tam; i++){

        actual=getNodoSiguiente(actual);
        insertarUltimo(listaCopia, getDato(actual));
    }
